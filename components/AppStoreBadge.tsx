import React from "react";

type Props = {
  href?: string | null;
  betaUrl?: string | null;
};

export function AppStoreBadge({ href, betaUrl }: Props) {
  const isBeta = !href && betaUrl;

  const inner = (
    <>
      {/* Apple Logo SVG */}
      <svg
        viewBox="0 0 24 24"
        className="w-5 h-5 fill-current text-white shrink-0"
        aria-hidden="true"
      >
        <path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701" />
      </svg>
      {/* Badge Text */}
      <div className="flex flex-col items-start leading-none gap-0.5">
        <span className="text-[8px] font-bold uppercase tracking-widest text-zinc-400">
          {href ? "Download on the" : isBeta ? "Join the open" : "Coming soon to"}
        </span>
        <span className="text-xs font-semibold text-white">
          {href ? "App Store" : isBeta ? "TestFlight Beta" : "App Store"}
        </span>
      </div>
    </>
  );

  const baseClass =
    "inline-flex items-center gap-3 bg-zinc-950 dark:bg-black text-white px-4 py-2 rounded-xl border border-zinc-800 hover:scale-[1.02] hover:border-zinc-700 transition-all select-none shadow-md";

  if (href || betaUrl) {
    return (
      <a
        href={(href || betaUrl) ?? undefined}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={href ? "Download on the App Store" : "Join the open TestFlight Beta"}
        className={baseClass}
      >
        {inner}
      </a>
    );
  }

  return <div className={baseClass}>{inner}</div>;
}
